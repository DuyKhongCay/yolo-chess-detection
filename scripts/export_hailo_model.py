...
def export_hailo_model(cfg: ExportHailoConfig):
    ...
    # STAGE: all (Full pipeline execution from ONNX parse)
    else:  # stage == "all"
        assert os.path.isfile(cfg.model_path), "Please provide valid path for model file"
    ...
    if stage in ("all", "optimize", "quantize"):
        ...
        if not input_layers:
            ...
            input_layer_name = input_layers[0]
